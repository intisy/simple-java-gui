...
import io.github.intisy.gui.javafx.ButtonBase;
import io.github.intisy.gui.javafx.Colors;
import io.github.intisy.gui.javafx.ExpandableString;
...
public class BasicButton extends ButtonBase {
    ...
    public void setCurrentTexts(ExpandableString texts) {
        ...
        while (true) {
            ...
            if (current.isExpanded()) {
                current = current.getSubStrings().get(0);
                xOffset += 10;
            } else {
                ExpandableString parent = current.getParent();
                if (parent != null) {
                    int i = parent.getSubStrings().indexOf(current) + 1;
                    if (parent.getSubStrings().size() > i)
                        current = parent.getSubStrings().get(i);
                    else {
                        ExpandableString parent2 = parent.getParent();
                        if (parent2 != null) {
                            i = parent2.getSubStrings().indexOf(parent) + 1;
                            if (parent2.getSubStrings().size() > i) {
                                current = parent2.getSubStrings().get(i);
                                xOffset -= 10;
                            } else
                                break;
                        } else
