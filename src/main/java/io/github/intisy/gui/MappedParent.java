...
public class MappedParent extends LayeredParent {
    private final ObservableMap<String, Doublet<Integer, Node>> children = FXCollections.observableHashMap();
    public MappedParent() {
        children.addListener((MapChangeListener<String, Doublet<Integer, Node>>) change -> {
            if (change.wasAdded()) {
    ...
    public void toFront(String key) {
        Doublet<Integer, Node> value = children.get(key);
        children.remove(key);
    ...
    public void add(String key, Node child, int layer) {
        ...
        children.put(key, new Doublet<>(layer, child));
    }
    public void renderOutline() {
        ObservableMap<String, Doublet<Integer, Node>> childrenClone = FXCollections.observableHashMap();
    ...
    public Map<String, Node> getChildrenMap() {
        Map<String, Doublet<Integer, Node>> sortedMap = new TreeMap<>(Comparator.reverseOrder());
